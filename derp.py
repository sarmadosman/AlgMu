...
# Use only the most recent N notes of the last phrase for harmony/voicing
...
# -----------------------------
# Less-jazzy controls (NEW)
# -----------------------------
MAX_LEAP = 3                 # smaller = smoother, less "jazzy"
MELODY_BASE_CHANCE = 0.95    # lower = less busy
CHORD_STAB_CHANCE = 0.15     # lower = less "comping"
STRAIGHT_RHYTHM = False       # True = more even timing
STRAIGHT_STEP = "eighth"     # "eighth" or "quarter"
...
@dataclass
class ChordState:
    held: Set[int] = field(default_factory=set)  # currently held input notes
    root_pc: int = 0
    scale_pcs: List[int] = field(default_factory=lambda: [0, 2, 4, 5, 7, 9, 11])  # default: major
...
# LESS-JAZZY: choose Major or Natural Minor instead of Dorian/Mixolydian
...
def sleep_for_step(i: int):
    if STRAIGHT_RHYTHM:
        if STRAIGHT_STEP == "quarter":
            time.sleep(60.0 / tempo)
        else:
            time.sleep((60.0 / tempo) / 2.0)  # eighth notes
        return

    # original uneven pattern timing
    if i in (0, 3):
        seconds = 16.0 / tempo
    elif i in (2, 5):
        seconds = 20.0 / tempo
...
# -----------------------------
# MIDI input thread (phrase handling + sustain)
# -----------------------------
...
def main():
    ...
    try:
        while True:
            ...
            for i in range(6):
                held_now, root_pc, scale_pcs, _, _, _, _, last_phrase_notes, phrase_id = snapshot_state()
                allow_piano = piano_enabled()

                # Reseed piano/harmony from newest phrase
                ...
                # If user starts playing again, silence generated piano once
                ...
                # -------------------------
                # Piano melody (less jazzy)
                # -------------------------
                if PLAY_PIANO and allow_piano:
                    delta = piano_patterns[piano_pat][i]
                    if delta is not None and chance(MELODY_BASE_CHANCE):
                        # clamp leaps to make it smoother
                        delta = max(-MAX_LEAP, min(MAX_LEAP, delta))
                        ...
                        note_on(piano_out, piano_note, 35 + rand_int(45))  # slightly narrower dynamics
                        active_melody_note = piano_note

                # -------------------------
                # Chord stabs (reduced)
                # -------------------------
                if PLAY_PIANO and allow_piano and i == 4 and chance(CHORD_STAB_CHANCE):
                    for n in active_harmony_notes:
                        note_off(piano_out, n)
                    active_harmony_notes.clear()

                    if last_phrase_notes:
                        src = last_phrase_notes[-RECENT_PHRASE_NOTES:]
                        chord_pcs = sorted({pc(n) for n in src})
                    elif held_now:
                        chord_pcs = sorted({pc(n) for n in held_now})
                    else:
                        chord_pcs = [(root_pc + s) % 12 for s in (0, 4, 7)]  # simple major triad fallback
                    ...
                    for p in chosen:
                        ...
                        note_on(piano_out, n, 20 + rand_int(35))
