...
# If True, piano waits for the source phrase to end before responding.
# If False, piano plays continuously over the MIDI file.
PHRASE_GATED_PIANO = False
...
# Input MIDI file is read from the command line.
# Example:
#   python full_code_midi_file_to_ableton.py input.mid
INPUT_MIDI_FILE = sys.argv[1] if len(sys.argv) > 1 else None

# These virtual MIDI outputs can be routed into Ableton.
...
# How tightly the generated piano follows the source register
REGISTER_HALF_SPAN = 8   # ±8 semitones around source center
MIN_REGISTER_WIDTH = 12  # at least one octave

CH = 0  # channel used for outgoing messages
POST_INPUT_TAIL_SECONDS = 2.0  # let the accompaniment ring out briefly after file ends
...
def build_scale_from_notes(root_pc: int, notes: Set[int]) -> List[int]:
    ...
    if has_M3:
        return [0, 2, 4, 5, 7, 9, 10]   # Mixolydian-ish
    return [0, 2, 3, 5, 7, 9, 10]       # Dorian-ish
...
def get_recent_source_notes(
    last_phrase_notes: List[int],
    current_phrase_notes: List[int],
    held_now: Set[int],
) -> List[int]:
    if current_phrase_notes:
        recent = current_phrase_notes[-RECENT_PHRASE_NOTES:] if RECENT_PHRASE_NOTES > 0 else current_phrase_notes
        return list(recent)
    if last_phrase_notes:
        recent = last_phrase_notes[-RECENT_PHRASE_NOTES:] if RECENT_PHRASE_NOTES > 0 else last_phrase_notes
        return list(recent)
    if held_now:
...
def piano_enabled() -> bool:
    ...
    now = time.monotonic()
    with state_lock:
        source_active = bool(state.held) or (USE_SUSTAIN_PEDAL and state.sustain_down)
        if source_active:
...
# -----------------------------
# MIDI input thread (reads MIDI FILE; phrase handling + sustain)
# -----------------------------
def midi_input_thread():
    try:
        mid = mido.MidiFile(INPUT_MIDI_FILE)
    except Exception as exc:
        print(f"Failed to open MIDI file: {exc}")
        input_finished.set()
        return

    print(f"Reading MIDI file: {INPUT_MIDI_FILE}")
...
def main():
    ...
    if not INPUT_MIDI_FILE:
        print("Usage: python full_code_midi_file_to_ableton.py <input.mid>")
        sys.exit(1)

    random.seed(seed)
    print(f"Running with tempo={tempo} BPM")
    print(f"Piano gated mode: {PHRASE_GATED_PIANO}")
    print("Creating virtual MIDI outputs for Ableton:")
    ...
    piano_was_on = False
    input_finished_at: Optional[float] = None
    ...
    try:
        while True:
            ...
            for i in range(6):
                ...
                # Once the source MIDI has ended, stop after a short tail.
                if input_finished.is_set() and not held_now:
                    if input_finished_at is None:
                        input_finished_at = time.monotonic()
                    elif time.monotonic() - input_finished_at >= POST_INPUT_TAIL_SECONDS:
                        return
                else:
